...
public class Inicio extends GameCanvas {
    ...
    private Imagenes fondoInicio1,fondoInicio2,fondoInicio3,
                     ...
                     modoHistoria,modoLibre,tituloModoLibre,
                     tituloModoHistoria,tituloRegresar;
    private Menu menu;
    ...
    public void dibujar() {
        ...
        if (fondoInicio4 != null) {
            ...
            if(fondoInicio4.getX() >= 79) {
                modoLibre.dibujar(g);
                if(highLight.getPrioridad() == modoLibre.getPrioridad()) {
                    tituloModoLibre.dibujar(g);
                }
                modoHistoria.dibujar(g);
                if(highLight.getPrioridad() == modoHistoria.getPrioridad()) {
                    tituloModoHistoria.dibujar(g);
                }
                regresar.dibujar(g);
                if(modoLibre.getY() <= 20) {
    ...
    public void crearImagenes() {
        try {
            ...
            tituloModoLibre = new Imagenes("/tituloModoLibre.png",100,100,40);
            tituloModoHistoria = new Imagenes("/tituloModoHistoria.png",101,100,165);
            tituloRegresar = new Imagenes("/tituloRegresar.png",102,20,100);
	} catch(IOException e){
            e.printStackTrace();
        }
    }
